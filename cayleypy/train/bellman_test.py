...
def test_target_can_be_an_ensemble_of_q_models():
    """Test that a predictor wrapping Q-models can be the target, which is what makes fine-tuning composable."""
    graph = _lrx5()
    n_generators = graph.definition.n_generators
    members = [Predictor(graph, _QModel()), Predictor(graph, _QModel())]
    ensemble = EnsemblePredictor(members)
    states = graph.random_walks(width=4, length=3)[0]

    data = bellman_targets(graph, states, ensemble, n_outputs=n_generators)

    assert data.targets.shape == (states.shape[0], n_generators)

    # The ensemble averages the outputs of its members, so its targets are the targets of a model returning that
    # average. (They are not the average of the members' targets: the Bellman operator takes a minimum, which does not
    # commute with averaging.)
    class _AveragedQModel(torch.nn.Module):
        n_outputs = n_generators

        def forward(self, states: torch.Tensor) -> torch.Tensor:
            return sum(member.predict_batched(states) for member in members) / len(members)

    expected = bellman_targets(graph, states, Predictor(graph, _AveragedQModel()), n_outputs=n_generators)
    assert torch.allclose(data.targets, expected.targets)
